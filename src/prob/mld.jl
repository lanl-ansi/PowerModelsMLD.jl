...
function post_mld(pm::GenericPowerModel)
...
function post_mld_uc(pm::GenericPowerModel)
...
function get_mld_solution(pm::GenericPowerModel, sol::Dict{String,Any})
...
function add_bus_status_setpoint(sol, pm::GenericPowerModel)
...
function run_mld_smpl(pm::GenericPowerModel)
    ...
    var(pm)[:vm_vio] = @variable(pm.model, vm_vio[i in ids(pm, :bus)] >= 0)
    var(pm)[:pg_vio] = @variable(pm.model, pg_vio[i in ids(pm, :gen)] >= 0)
    vm = var(pm, :vm)
    pg = var(pm, :pg)
    qg = var(pm, :qg)

    z_demand = var(pm, :z_demand)
    z_shunt = var(pm, :z_shunt)

    M = maximum([abs(load["pd"]) for (i,load) in ref(pm, :load)])
    @objective(pm.model, Max,
        sum( -10*M*vm_vio[i] for (i, bus) in ref(pm, :bus)) +
        sum( -10*M*pg_vio[i] for i in ids(pm, :gen) ) +
        sum( M*z_shunt[i] for (i, shunt) in ref(pm, :shunt)) +
        sum( abs(load["pd"])*z_demand[i] for (i, load) in ref(pm, :load))
    )
    ...
    for (i, bus) in ref(pm, :bus)
        ...
        @constraint(pm.model, vm[i] <= bus["vmax"] + vm_vio[i])
        @constraint(pm.model, vm[i] >= bus["vmin"] - vm_vio[i])
    end

    for (i, gen) in ref(pm, :gen)
        @constraint(pm.model, pg[i] <= gen["pmax"] + pg_vio[i])
        @constraint(pm.model, pg[i] >= gen["pmin"] - pg_vio[i])

        @constraint(pm.model, qg[i] <= gen["qmax"])
        @constraint(pm.model, qg[i] >= gen["qmin"])
    end
...
function get_mld_smpl_solution(pm::GenericPowerModel, sol::Dict{String,Any})
    PMs.add_bus_voltage_setpoint(sol, pm)
    PMs.add_generator_power_setpoint(sol, pm)
    PMs.add_branch_flow_setpoint(sol, pm)
    add_load_setpoint(sol, pm)
